...
"""ANOVA 결과(p < 0.05), 해당 부서에서 퇴직 여부에 따라 평균 만족도에 통계적으로 유의한 차이가 있는 것으로 나타났다. 이는 만족도가 퇴직 여부를 설명하는 핵심 요인일 가능성을 시사한다.
...
이때 모델링 단계에서는 다변량 환경에서 변수 간 중복 설명을 고려하기 위해 L2 정규화( Ridge)가 적용된 로지스틱 회귀 모델을 사용할 것이며, 비선형 관계를 자동으로 학습하는 Random Forest 모델과의 성능 비교를 통해 예측력을 추가로 검증할 것이다.
...
해당 모델은 소수 클래스(퇴직자)에 대해 precision 0.85, recall 0.84, F1-score 0.85를 기록하였으며, ROC-AUC는 0.96으로 높은 분류 성능을 보였다. 이는 선형 가정 하에서도 주요 예측 신호가 충분히 포착되고 있음을 의미한다.

특히 클래스 불균형 상황에서도 macro average 지표가 안정적으로 유지되어, 모델이 특정 클래스에 편향되지 않고 균형 잡힌 예측을 수행하고 있음을 확인하였다. 본 모델은 해석 가능성과 예측 성능 간의 균형 측면에서 합리적인 기준 모델(baseline)로 활용 가능하다.
"""
...
Random Forest 모델은 소수 클래스(퇴직자)에 대해 precision 0.99, recall 0.92, F1-score 0.95를 기록하여, 퇴직자 탐지 성능이 전반적으로 향상되었음을 확인하였다. 정확도는 0.98로 상승하였으며, 다수·소수 클래스 모두에서 높은 분류 성능을 보였다.

이는 일부 변수 조합에서 비선형적 패턴이 존재함을 시사하며, 보다 복잡한 의사결정 경계를 통해 오분류를 감소시킬 수 있음을 보여준다. 다만 모델 구조의 특성상 해석 가능성은 상대적으로 제한된다.
...
로지스틱 회귀 모델은 높은 ROC-AUC(0.96)와 안정적인 분류 성능을 통해, 주요 예측 신호가 비교적 선형적인 구조를 갖고 있음을 보여주었으며, 해석 가능성 측면에서 강점을 가진다. 반면, Random Forest 모델은 소수 클래스에 대한 재현율과 F1-score를 추가적으로 개선하여, 변수 간 비선형 상호작용이 일부 존재함을 시사한다.

이에 따라 본 프로젝트에서는 설명 목적 및 정책 해석이 중요한 경우 로지스틱 회귀 모델을, 예측 정확도 극대화가 요구되는 경우 Random Forest 모델을 선택할 수 있는 근거를 확보하였다. 이는 모델 선택 과정에서 성능뿐 아니라 해석 가능성을 함께 고려한 합리적인 의사결정임을 보여준다.
"""

ls ~/.ssh

!ssh-keygen -t ed25519 -C "sjyeee8704@gmail.com"
...
!bash -lc 'eval "$(ssh-agent -s)" >/dev/null && ssh-add -l'

!ssh -i ~/.ssh/id_ed25519 -o IdentitiesOnly=yes -o StrictHostKeyChecking=no -T git@github.com

!bash -lc 'eval "$(ssh-agent -s)" && ssh-add ~/.ssh/id_ed25519 && ssh-add -l && ssh -T git@github.com'

!git remote -v
!git status
!git add .
!git commit -m "Update portfolio modeling results"
!git push -u origin main

!git init
!git branch -M main

!git remote add origin git@github.com:sungjihe/<HR>.git

!git config --global user.email "sjyeee0874@gmail.com"
!git config --global user.name "sungjihe"
!git add .
!git commit -m "Initial commit: HR"
!git push -u origin main

!pwd
!ls

!mkdir -p /content/hr-portfolio
!cp -r /content/HR /content/hr-portfolio/
!cp /content/HR.csv /content/hr-portfolio/

# Commented out IPython magic to ensure Python compatibility.
# %cd /content/hr-portfolio
!git init
!git branch -M main
...
!git add .
!git status
!git commit -m "Initial commit: HR portfolio analysis"
...
!GIT_SSH_COMMAND='ssh -i ~/.ssh/id_ed25519 -o IdentitiesOnly=yes -o StrictHostKeyChecking=no' git ls-remote origin

!cat ~/.ssh/id_ed25519.pub

!ssh -vvv -i ~/.ssh/id_ed25519 -o IdentitiesOnly=yes -T git@github.com

!rm -f ~/.ssh/id_ed25519 ~/.ssh/id_ed25519.pub
!ssh-keygen -t ed25519 -C "sjyeee8704@gmail.com" -f ~/.ssh/id_ed25519 -N ""
!cat ~/.ssh/id_ed25519.pub

!ssh -i ~/.ssh/id_ed25519 -o IdentitiesOnly=yes -o StrictHostKeyChecking=no -T git@github.com

!GIT_SSH_COMMAND='ssh -i ~/.ssh/id_ed25519 -o IdentitiesOnly=yes -o StrictHostKeyChecking=no' git push -u origin main

!GIT_SSH_COMMAND='ssh -i ~/.ssh/id_ed25519 -o IdentitiesOnly=yes -o StrictHostKeyChecking=no' git pull origin main --allow-unrelated-histories
